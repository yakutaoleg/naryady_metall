...
LOG_FILE = '/root/naryady/prod/logs/naryady.log'
...
def alert(msg: str):
    ...
    if _bot_instance is not None:
        # asyncio-путь (бот запущен в том же процессе)
        async def _send():
            try:
        ...
        return
    # Fallback: subprocess (sync.py) — отправляем через HTTP напрямую
    try:
        import urllib.request, urllib.parse, os as _os
        token = _os.environ.get('TG_TOKEN', '')
        if not token:
            from src import config as _cfg
            token = _cfg.TG_TOKEN
        payload = urllib.parse.urlencode({'chat_id': ADMIN_CHAT_ID, 'text': text, 'parse_mode': 'HTML'}).encode()
        req = urllib.request.Request(f'https://api.telegram.org/bot{token}/sendMessage', data=payload)
        urllib.request.urlopen(req, timeout=5)
    except Exception as e:
        log.error(f"alert http fallback failed: {e}")
