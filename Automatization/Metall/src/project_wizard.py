...
def register_project(
    ...
    # Сохраняем зависимости в project_deps если таблица существует
    try:
        for a_spec, b_spec in deps:
            db.execute(
                """INSERT INTO project_deps (project_id, spec_from, spec_to)
                   VALUES (%s, %s, %s) ON CONFLICT DO NOTHING""",
                [project_id, a_spec, b_spec]
            )
    except Exception as e:
        log.warning(f'register_project: project_deps не сохранены (таблица не существует?): {e}')
