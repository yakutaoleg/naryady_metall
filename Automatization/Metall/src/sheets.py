...
def update_task_status(file_id: str, sheet_name: str, row_num: int,
    ...
    """Write status/comment/date_fact/qty_done back to Google Sheets using dynamic column lookup."""
    ...
    col_qty_done = col_map.get('ВЫПОЛНЕНО')
    if qty_done is not None and col_qty_done:
        ws.update(values=[[qty_done]], range_name=f'{_col_letter(col_qty_done)}{sheet_row}')
