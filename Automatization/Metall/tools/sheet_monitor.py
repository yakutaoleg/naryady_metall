...
def check_sheet(sheet_name, cfg):
    ...
    for idx in range(2, data_end):
        ...
        expected_blok = 'СВЕРЛЕНИЕ' if sheet_name.upper() in NEXT_SPEC else ''
...
def main():
    ...
    # Правило A: зависания БЛОК + здоровье автогенерации
    try:
        project = db.fetchone(
        ...
        if project:
            ...
            all_issues.extend(check_deps_health(project['project_name']))
    except Exception as e:
